...
- [ ] Replace direct static `R::` calls in `BruteForceShield` and `AuthDatabaseInit` with repositories/models.
- [ ] Use `Assembler` DI container in `AuthServiceProvider` instead of manual `new` instantiation.
- [ ] Convert manual route array registrations to native PHP 8.5 `#[Route]` attributes.
- [ ] Handle HTMX requests in `AuthController` using `HX-Redirect` headers.
